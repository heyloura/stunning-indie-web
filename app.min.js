...
const _app = {
  ...
  function post() {
    ...
    section.setAttribute(_app.post.attributeIds.tags, _app.post.getTags());
  ...
  function loadStream() {
    // clean the DOM
    let stream = byId("stream");
    stream.innerHTML = "";
  
    // get the sections and sort them
    var sections = document.querySelectorAll(`section[${_app.post.attributeIds.type}='${_app.sectionTypes.post}'][${_app.post.attributeIds.date}]`); 
    if (sections != null && sections != undefined && sections.length > 0) {
      ...
      sections.forEach((section) => {
        ...
        if (content != null && content != undefined) {
          ...
          [].slice.call(children).forEach((child) => stream.append(child));
        }
      });
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {  
    ...
    //  ---------------------------------------------
    console.log('readonly', !_app.readonly())
    if (!_app.readonly()) {
      // remove the github PAT div to make the user log out
      byId(_app.htmlIds.settingsGithubPat).remove();

      // make auth buttons visible
      ...
      byId(_app.htmlIds.settingsGIthubAutoSave).remove();
    }
