...
const _app = {
    ...
    htmlIds: {
      ...
      webmention: 'configuration_indieauth_webmention',
      webmentionFeed: 'configuration_indieauth_webmention_feed',
      webmentionFeedPolled: 'configuration_indieauth_webmention_feed_polled',
      webmentionTelegraph: 'configuration_indieauth_webmention_telegraph'
    },
    indieweb: {
      getMyImg: function() { return byIdValue(_app.htmlIds.hcardimg) },
      getMyName: function() { return byIdValue(_app.htmlIds.hcardname) },
      getTelegraphApiKey: function() { return byIdValue(_app.htmlIds.webmentionTelegraph) },
    },
    github: {
...
//https://gomakethings.com/a-native-vanilla-javascript-way-to-get-the-closest-matching-parent-element/
  ...
  function saveTelegraphAPIKey() {
    var value = _app.indieweb.getTelegraphApiKey();
    if (value) {
      localStorage.setItem("small-web-telegraph", value);
    }
    location.reload();
  }

  function autoSave(msg = 'Auto save') { 
    if(byIdValue(_app.htmlIds.githubRepo)) {
      byId(_app.htmlIds.editorControls).innerHTML = '';
      
      authDisplay(false);
      var streams = bySelAll(`section[data-type="stream"]`);
      [].slice.call(streams).forEach((btn) => {
        streams.innerHTML = ''; 
      });
  ...
  function generatePostFooter(actionType, id, type) {
    ...
    return `<div class="article-actions">${actionLink} ${tagsDisp}</div>`;
  }
  ...
  function generatePostHTML(html, type, id, created, updated) {
    ...
    if (type === "reply") {
      const title = `in repy to <a href="${_app.post.getUrl()}" class="u-in-reply-to" target="_blank">${_app.post.getName()}</a>`;
      const footer = generatePostFooter('💬 reply', id, _app.postTypes.reply);

      return {
        title: `Replied to ${_app.post.getName()}`,
        body: `<article class="h-entry">
        <header>${authorDisp}${title}</header>
        <div class="e-content p-content">               
          <div class="post-content">${html}</div>
        </div>
        <footer>
          replied on ${publishedDisp} ${footer}
        </footer>
        </article>`
      };
    }
    ...
    if (type === "like") {
      const footer = generatePostFooter('👍 like', id, _app.postTypes.like);
      return {
        title: `Liked a post by ${_app.post.getName()}`,
        body: `<article class="h-entry">
        <div class="h-cite u-like-of">
          <header>Liked <a class="u-url" href="${_app.post.getUrl()}" target="_blank">a post</a> by
            <span class="p-author h-card liked-name">
            <a class="u-url p-name" href="${_app.post.getDomain()}">${_app.post.getName()}</a>
            </span>:</header>
            <blockquote class="e-content post-content">
                ${html}
            </blockquote>     
        </div>
        <footer>liked on ${publishedDisp} ${footer}</footer>        
        </article>`
      };
    }
  
    if (type === "bookmark") {
      const footer = generatePostFooter('🔖 bookmark', id, _app.postTypes.bookmark);
      return {
        title: `Bookmarked ${_app.post.getTitle()}`,
        body: `<article class="h-entry">${authorDisp}
          <header>
          ⧉ <a class="u-bookmark-of h-cite title" href="${_app.post.getUrl()}" target="_blank">
            ${_app.post.getTitle()}
          </a>
          </header>
          ...
          <footer>
          bookmarked on ${publishedDisp} ${footer}
          </footer>
        </article>`
      };
    }
    ...
    if (type === "article") {
      const footer = generatePostFooter('📃 article', id, _app.postTypes.article);
      return {
        title: `${_app.post.getTitle()}`,
        body: `<article class="h-entry">
          <header>
            <h1><a href="#${id}" class="p-name">${_app.post.getTitle()}</a></h1>
            ${authorDisp}
          </header>     
          <p class="p-summary">
            ${_app.post.getSummary()}
            <br/><a href="#${id}">Read More ...</a>
          </p>
          <div class="e-content post-content">${html}</div>
          <footer>
          published on ${publishedDisp}
          ${footer}
          </footer 
        </article>`
      };
    ...
    // default is a note
    const footer = generatePostFooter('📔 note', id, _app.postTypes.note);
    return {
        title: `Note on ${dateTimeDisp}`,
        body: `<article class="h-entry">
            <header>${authorDisp}</header>
  ...
  function post() {
    ...
    const html = DOMPurify.sanitize(_app.post.getPost(), {FORBID_ATTR: ['style']});
    ...
    if(!editId)
    {
      ...
      liLink.setAttribute("href", '#' + urlHash);
      liLink.innerHTML = `${data.title}`;
      li.prepend(liLink);
      byId(_app.htmlIds.index).prepend(li);
    }
  ...
  function edit(id) {
    ...
    if (postEl) {
      let content = '';
      if(bySel(`[id='${id}-content'] > .h-entry > .post-content`)){
        content = bySel(`[id='${id}-content'] > .h-entry > .post-content`).innerHTML;
      } else {
        content = Base64.decode(postEl.getAttribute(_app.post.attributeIds.content));
      }     
  ...
  function deleteMe(id) {
    if(confirm("Are you sure you want to delete this?")) {
      byId(id).remove();
      bySel(`[id='index'] > li > a[href='#${id}']`).closest('li').remove();
      autoSave('Removed Post');
      window.location.hash = "stream";
    }
  }

  function loadPostType(type) {
    type = type == null || type == undefined || type == '' ? "stream" : type;
    // clean the DOM
    let posts = byId(type);
    posts.innerHTML = "";
  ...
  let authDisplay = function(show) {
    ...
    if(show) {
      dispNone(byId(_app.htmlIds.settingsGithubPat))
      if(localStorage.getItem("small-web-telegraph")){
        dispNone(byId("telegraph_apikey_input"))
      }
    }
    else {
      dispBlock(byId(_app.htmlIds.settingsGithubPat))
      dispBlock(byId("telegraph_apikey_input"))
    }
  ...
  document.addEventListener("DOMContentLoaded", function () {  
    ...
    bySelAll("input.config").forEach((input) =>
      input.addEventListener("input", function () {
        ...
        if (this.getAttribute("id") === _app.htmlIds.webmention) {
          var element = byId("webmention");
